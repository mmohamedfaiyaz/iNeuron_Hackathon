            ...
            if (err) {
                ...
                this.setState({
                    supplierContract_blockchainRecordedItemIds: [...this.state.supplierContract_blockchainRecordedItemIds,
                        parseInt(eventLogs.args.idItem.toString(),10)
                    ]
                });
            }
        });
            ...
            if (err) {
                ...
                this.setState({
                    customerContract_blockchainRecordedPurchaseOrderIds: [...this.state.customerContract_blockchainRecordedPurchaseOrderIds,
                        parseInt(eventLogs.args.idOrder.toString(),10)
                    ]
                });
            }
        });
        ...
        this.supplierContract_processAnOrderEvents.watch((err, eventLogs) => {
            if (err) {
                ...
                if (eventLogs.args.status){
                    this.customerContract_recieveItem(parseInt(eventLogs.args.idOrder.toString(),10));
                }
            }
        })
    }
                                ...
                                <Panel.Body>
                                    <Tabs defaultActiveKey={1} id="uncontrolled-tab-example">
                                        <Tab eventKey={1} title="Market">
                                            {this.state.supplierContract_blockchainRecordedItemIds.map(itemId =>{let itemDetails=this.supplierContract_getItem(itemId);
                                                return (
                                                    <div>
                                                        <Panel onClick={() => this.purchaseThisItem({
                                                            'id': itemId,
                                                            'itemName': web3.toUtf8(String(itemDetails).split(',')[0]),
                                                            'price': parseInt(String(itemDetails).split(',')[1],10),
                                                            'quantity': 1
                                                        })}>
                                                            <Panel.Heading className="pointIt">HOT SALE! <small>Click to purchase!</small></Panel.Heading>
                                                            <Panel.Body>
                                                                {web3.toUtf8(String(itemDetails).split(',')[0])} @ ${parseInt(String(itemDetails).split(',')[1],10)}
                                                            </Panel.Body>
                                                        </Panel>
                                                    </div>
                                                );
                                        ...
                                        <Tab eventKey={2} title="Order(s)">
                                            ...
                                            <Table striped bordered condensed hover>
                                                ...
                                                <tbody>
                                                    {
                                                    [...new Set(this.state.customerContract_blockchainRecordedPurchaseOrderIds)].map(orderId=>{const orderDetails=this.customerContract_getOrderDetails(orderId);
                                                        return (<tr>
                                                            <td>
                                                            {orderId}
                                                            </td>
                                                            ...
                                                            <td>
                                                            {parseInt(String(orderDetails).split(',')[1],10)}
                                                            </td>
                                                            <td>
